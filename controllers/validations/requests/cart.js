...
module.exports =
{
  cart: async (Cart) => {
    ...
    const { value: ShippedValue, quantity: quantityShipped } = await shipped(Cart)

    return Boolean(realValue == ShippedValue && quantityReal == quantityShipped)

    async function real (Cart) {
      let results; let value = quantity = 0
      ...
      (
        Cart.map(async iterator => {
          ...
          const Variations = await variations.findById(iterator.variations)
