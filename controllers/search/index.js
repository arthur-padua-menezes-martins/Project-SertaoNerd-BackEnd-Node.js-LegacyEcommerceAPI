...
const classExtensions = require('../../helpers/classExtensions/index.js')

class searchController {
  async searchRequests (request, response, next) {
    try {
      const { params } = request
      var query = { $or: [{ title: { $regex: new RegExp(params.categories, 'i') } }, { description: new RegExp(params.name, 'i') }] }; var offset = Number(params.offset || 0); var limit = Number(params.limit || 30); var error = new Error()
  ...
  async searchProducts (request, response, next) {
    try {
      ...
      var paginationQuery = { availability: true }; var options = {}; var select; var selectType; var _id = []; var names = new Set(); var types = new Set(); var collections = []
  ...
  async searchTerm (request, response, next) {
    try {
      const { params, query } = request; const { searchTerm } = params; const searchTermIn = (searchTerm.split(' ')).filter(term => term !== '')
      var options = {}
