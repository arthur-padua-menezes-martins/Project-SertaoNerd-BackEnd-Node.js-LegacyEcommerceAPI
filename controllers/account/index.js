...
class accountController {
  ...
  async update (request, response, next) {
    try {
      ...
      const { body, session } = request; const { user } = session; const { _id } = user
      const update = {}; const keys = []

      users.findById(_id).then(async (User) => {
        for (const Key in body) { body[Key] ? update[Key] = body[Key] : keys.push(Key) }
  ...
  async delete (request, response, next) {
    try {
      const { session } = request; const { user } = session; const { _id } = session
