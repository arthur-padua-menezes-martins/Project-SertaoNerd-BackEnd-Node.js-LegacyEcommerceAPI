...
/* VALIDATIONS MODULES */
const validationsAccount = require('../validations/account/account.js')

/* FEATURES MODULES */
const recoverySendingEmail = require('../../controllers/sendingEmail/recovery.js')

class usersController {
  async verifySession (request, response, next) {
    try {
      const { params, headers, sessionStore } = request
      let aux; const accountInformations = {}; const sessionID = headers.cookie.slice((headers.cookie).search('@sessionID=') + 11, (headers.cookie).indexOf(';'))
  ...
  async recovery (request, response, next) {
    try {
      const { email } = request.body; const recovery = new Date().getTime() + 1000 * 60 * 60 * 24 * 2
